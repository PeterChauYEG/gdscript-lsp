...
mod tests {
        ...
        for i in 0..root.child_count() as u32 {
            ...
            for j in 0..stmt.child_count() as u32 {
                ...
                if child.kind() == "=" {
                    ...
                    return infer_literal_type(&child);
                }
            }
        }
        None
    ...
    #[test]
    fn int_literal() {
        assert_eq!(literal_type("var x = 5\n"), Some("int"));
    }

    #[test]
    fn float_literal() {
        assert_eq!(literal_type("var x = 3.14\n"), Some("float"));
    }

    #[test]
    fn string_literal() {
        assert_eq!(literal_type("var x = \"hi\"\n"), Some("String"));
    }

    #[test]
    fn bool_true() {
        assert_eq!(literal_type("var x = true\n"), Some("bool"));
    }

    #[test]
    fn bool_false() {
        assert_eq!(literal_type("var x = false\n"), Some("bool"));
    }

    #[test]
    fn null_literal() {
        assert_eq!(literal_type("var x = null\n"), Some("Nil"));
    }

    #[test]
    fn expression_not_literal() {
    ...
    fn same_type_compatible() {
        let d = db();
        assert!(types_compatible("int", "int", &d));
    }

    #[test]
    fn int_to_float() {
        let d = db();
        assert!(types_compatible("float", "int", &d));
    }

    #[test]
    fn variant_accepts_string() {
        let d = db();
        assert!(types_compatible("Variant", "String", &d));
    }

    #[test]
    fn string_to_int_incompatible() {
        let d = db();
        assert!(!types_compatible("int", "String", &d));
    }
